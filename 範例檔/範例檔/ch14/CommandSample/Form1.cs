...
namespace CommandSample
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // 庫存主檔BindingSource搜尋供應商代號
            //等於cboVendorName.SelectedValue的資料
        ...
        //當供應商名稱cboVendorName清單被選取之後
        //會執行cboVendorName_SelectedIndexChanged事件處理函式
        ...
        private void btnAddBuy_Click(object sender, EventArgs e)
        {
            ...
            {
                // 連接db1.mdb資料庫
                OleDbConnection cn = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=db1.mdb");
                cn.Open();
                // 建立OleDbCommand的cmd物件
                OleDbCommand cmd = new OleDbCommand();
                cmd.Connection = cn;             // cmd物件連接db1.mdb
                //立SQL INSERT敘述，該敘述可將進貨資料寫入'進貨明細'資料表
                cmd.CommandText = "INSERT INTO 進貨明細(供應商代號,品名代號," +
                "進價,數量,備註,進貨日期)VALUES('" +
                ...
                cmd.CommandText = "UPDATE 庫存主檔 SET 庫存量=庫存量+" +
                    Convert.ToInt32(txtQty.Text) + " WHERE 品名代號='" +
                    cboProductName.SelectedValue.ToString() + "'";
                cmd.ExecuteNonQuery();  // 執行SQL敘述
